...
def list_shelters(db: Session = Depends(get_db)) -> list[ShelterResponse]:
    ...
    return list(db.scalars(select(Shelter).order_by(Shelter.name)).all())
...
def road_conditions(db: Session = Depends(get_db)) -> list[RoadConditionResponse]:
    if settings.demo_mode:
        return demo_roads()
    return list(db.scalars(select(RoadCondition).order_by(RoadCondition.last_updated.desc())).all())
